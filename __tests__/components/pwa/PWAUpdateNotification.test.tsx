...
describe("PWAUpdateNotification", () => {
  ...
  it("shows notification when updatefound event fires with new worker", async () => {
  ...
  it("does not show notification when new worker state is not installed", async () => {
    let updateFoundCallback: (() => void) | null = null;
    let stateChangeCallback: (() => void) | null = null;
    ...
    // Simulate statechange (but state is not "installed")
    if (stateChangeCallback) {
      stateChangeCallback();
    }
  ...
  it("handles updatefound when installing worker is null", async () => {
    ...
    // Should not crash and should not show update
    expect(screen.queryByText("Update Available")).not.toBeInTheDocument();
  });
